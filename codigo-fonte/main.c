...
// Estrutura para armazenar um vetor em 2D
typedef struct {
    double x;
    double y;
} Vetor; 
...
double calcularAngulo(Vetor u, Vetor v) {
    ...
    // Correção numérica para evitar erros de domínio em arcos
...
int main() {
    ...
    // Entrada dos vetores pelo usuário
    printf("Digite as coordenadas do vetor V1 = (x1, y1):\n");
    scanf("%lf %lf", &v1.x, &v1.y);
    printf("Digite as coordenadas do vetor V2 = (x2, y2):\n");
    scanf("%lf %lf", &v2.x, &v2.y);
    printf("Digite as coordenadas do vetor V3 = (x3, y3):\n");
    ...
    double ang1 = calcularAngulo(AB, AC);  // Ângulo no ponto A
    double ang2 = calcularAngulo(BA, BC);  // Ângulo no ponto B
    double ang3 = calcularAngulo(CA, CB);  // Ângulo no ponto C

    // Soma dos ângulos (permite tolerância para evitar erros de precisão)
    double somaAngulos = ang1 + ang2 + ang3;
    if (fabs(somaAngulos - 180.0) > 1e-6) {
        printf("Os vetores não formam um triângulo válido (soma dos ângulos: %.2f).\n", somaAngulos);
        return 1;
    }

    // Cálculo dos lados (módulos dos vetores)
